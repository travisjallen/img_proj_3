...
if ((np.amax(sizes[:,0]) != np.amin(sizes[:,0])) or (np.amax(sizes[:,1]) != np.amin(sizes[:,1]))):
    ...
    pairs_exact[:,0:5] = pairs[0:row_count,:]
    ...
    for i in range(row_count):
        ...
        ## Determine the placement coordinates with respect to the first image argument
        F_I = overlap(pc,img_array[:,:,int(pairs_exact[i,0])],img_array[:,:,int(pairs_exact[i,1])])

        ## store the coordinates in pairs_exact
        pairs_exact[i,5] = F_I[0]
        pairs_exact[i,6] = F_I[1]

    ## define a canvas
    canvas_width = int(5*np.amax(sizes[:,1]))
    canvas_height = int(5*np.amax(sizes[:,0]))
    ...
    ## place image 0
    canvas[center_y:(int(center_y + sizes[0,0])),center_x : int(center_x + sizes[0,1])] = img_array[:,:,0]
    
    ## plot the ones we can
    for i in range(row_count):
        ## if the image is a pair with 0, plot it
        if ((pairs_exact[i,0] == 0) or (pairs_exact[i,1] == 0)):
            if (pairs_exact[i,5] != 0):
                ## calculate coordinates
                shift_y = int(pairs_exact[i,5])
                shift_x = int(pairs_exact[i,6])
                
                origin_y = int(center_y + shift_y)
                terminus_y = int(origin_y + sizes[0,1])
                
                origin_x = int(center_x + shift_x)
                terminus_x = int(origin_x + sizes[0,1])
                canvas[origin_y:terminus_y,origin_x:terminus_x] = img_array[:,:,i]

                ## mark that it has been plotted
                pairs_exact[i,4] = 1

    ## now plot the others
    for i in range(row_count):
        ## check if the image has already been plotted
        if (pairs_exact[i,4] != 1):
            parent_shift_y = 0
            parent_shift_x = 0
            parent_image = pairs_exact[i,1]

            ## find the offset of the parent image
            for j in range(row_count):
                if (pairs_exact[j,2] == parent_image):
                    parent_shift_y = pairs_exact[i,5]
                    parent_shift_x = pairs_exact[i,6]
            
            ## add the parent shift to the existing offsets
            pairs_exact[i,5] = pairs_exact[i,5] + parent_shift_y
            pairs_exact[i,6] = pairs_exact[i,6] + parent_shift_x
